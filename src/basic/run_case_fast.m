function STPC_results = run_case_fast(config)
% RUN_CASE_FAST  Main controller using the fast smoothing-comparison path.
%
...
% so repeated runs can distinguish recomputation from cache reads.
    ...
    stageTimer = tic;
    mssa_smooth_results = run_mssa_smooth_fast(basicInfo, slepianInfo, mssaInfo, ...
        slepian_results);
    elapsed = toc(stageTimer);
